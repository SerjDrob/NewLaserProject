...
internal class GeneralLaserProcess : BaseLaserProcess, IProcess
{
                ...
                _workCoorSystem = _fileAlignment switch
                {
                   ...
                   .Build(),
                    FileAlignment.AlignByTwoPoint => _pureCoorSystem
                        ...
                        .FormWorkMatrix(1, 1)
                        .Build(),
                    FileAlignment.AlignByCorner => _baseCoorSystem.ExtractSubSystem(LMPlace.FileOnWaferUnderLaser),
    ...
    public async Task Next()
    {
        try
        {
            ...
            else if(!_stateMachine.IsInState(State.Teaching)) 
            {
                _inProcess = true;
    ...
    public IDisposable Subscribe(IObserver<IProcessNotify> observer) => throw new NotImplementedException();
