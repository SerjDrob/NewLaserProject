...
namespace NewLaserProject.Classes.Process
{
    internal class GeneralLaserProcess : BaseLaserProcess, IProcess
    {
        ...
        public void CreateProcess()
        {
            ...
            _subject.OfType<SnapShotResult>()
                .Subscribe(result =>
                {
                    ...
                    _gotRefPoints = originPoints.Count == (_fileAlignment switch
                    {
        ...
        public async Task Next()
        {
            try
            {
                if (_stateMachine.IsInState(State.Teaching) && _fileAlignment != FileAlignment.AlignByCorner && !_gotRefPoints)
                {
                    await _stateMachine.FireAsync(Trigger.Next);
                }
                else if (_gotRefPoints || _fileAlignment == FileAlignment.AlignByCorner)
                {
                    _inProcess = true;
